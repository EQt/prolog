...
test(puzzle_with_three_letters, condition(pending)) :-
...
test(puzzle_with_ten_letters, condition(pending)) :-
    ...
    Solution == ['A'-5, 'D'-3, 'E'-4, 'F'-7, 'G'-8, 'N'-0, 'O'-2, 'R'-1, 'S'-6, 'T'-9].
...
test(puzzle_with_ten_letters_and_199_addends, condition(pending)) :-
    ...
    Solution == ['A'-1, 'E'-0, 'F'-5, 'H'-8, 'I'-7, 'L'-2, 'O'-6, 'R'-3, 'S'-4, 'T'-9].
